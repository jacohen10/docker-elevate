...
describe Restaurant do
  before(:each) do
    @restaurant = Restaurant.create(name: 'Mellow Mushroom', email: 'testing@email.com', phone: 7_578_488_208, order_ahead: true, in_restaurant_payment: true)
...
describe Restaurant do
  before(:each) do
    @restaurant = Restaurant.create(name: 'Mellow Mushroom', email: 'testing@email.com', phone: 7_578_488_208, order_ahead: true, in_restaurant_payment: true)
    expect(@restaurant).to be_a(Restaurant)
    @opentime = @restaurant.open_times.create(day: Time.zone.now.strftime('%A'), opening: (Time.zone.now - 5.hours), closing: (Time.zone.now + 2.hours))
  end
